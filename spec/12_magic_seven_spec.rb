...
describe MagicSeven do
  ...
  describe '#subtract_random_number' do
    context 'when the random number is 9' do
      it 'returns 1' do
        new_game = MagicSeven.new(random_number = 9)
        result = new_game.subtract_random_number(10)
  ...
  describe '#play' do
    context 'when the random number is 11' do
      ...
      it 'will return 7' do
        new_game = MagicSeven.new(random_number = 11)
    ...
    context 'when the random number is 0' do
      # remove the 'x' before running this test
      it 'will return 7' do
        new_game = MagicSeven.new(random_number = 0)
        result = new_game.play
        expect(result).to eq(7)
      end
    end
    ...
    context 'when the random number is 20' do
      # remove the 'x' before running this test
      it 'will return 7' do
        new_game = MagicSeven.new(random_number = 20)
        result = new_game.play
        expect(result).to eq(7)
      end
    end
