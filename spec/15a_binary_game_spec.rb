...
describe BinaryGame do
  ...
  describe '#player_input' do
    ...
    context 'when user inputs an incorrect value once, then a valid input' do
      before do
        invalid_input = '11'
        valid_input2 = '4'
        allow(game_input2).to receive(:gets).and_return(invalid_input, valid_input2)
      end
      ...
      it 'completes loop and displays error message once' do
        ...
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input2).to receive(:puts).with(error_message)
        expect(game_input2).not_to receive(:puts).with(error_message)
    ...
    context 'when user inputs two incorrect values, then a valid input' do
      before do
        ...
        invalid_input_3 = 'q'
        valid_input3 = '4'
        allow(game_input2).to receive(:gets).and_return(invalid_input_2, invalid_input_3, valid_input3)
      end
      ...
      it 'completes loop and displays error message twice' do
        ...
        max = game_input2.instance_variable_get(:@maximum)
        error_message = "Input error! Please enter a number between #{min} or #{max}."
        expect(game_input2).to receive(:puts).with(error_message).twice
        expect(game_input2).not_to receive(:puts).with(error_message)
        game_input2.player_input(min, max)
      end
    end
  ...
  describe '#verify_input' do
    ...
    subject(:binary_game_input) { described_class.new(1, 10) }

    # Note: #verify_input will only return a number if it is between?(min, max)

    context 'when given a valid input as argument' do
      it 'returns valid input' do
        result = binary_game_input.verify_input(1,10,5)
    ...
    context 'when given invalid input as argument' do
      it 'returns nil' do
        result2 = binary_game_input.verify_input(1,10,15)
  ...
  describe '#display_turn_order' do
    ...
    let(:turn) { instance_double(BinarySearch) }
    before do
      # You'll need to create a few method stubs.
      allow(turn).to receive(:make_guess)
      allow(turn).to receive(:update_range)
    ...
    it 'increases guess_count by one' do
      expect { game_turn.display_turn_order(turn) }.to change { game_turn.instance_variable_get(:@guess_count) }.by(1)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends make_guess' do
      expect(turn).to receive(:make_guess).once
      game_turn.display_turn_order(turn)
    end

    # Method with Outgoing Command -> Test that a message is sent
    it 'sends update_range' do
      expect(turn).to receive(:update_range).once
      game_turn.display_turn_order(turn)
    end
